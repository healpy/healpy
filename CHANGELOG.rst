...
* Added ``harmonic_ud_grade`` to change map NSIDE via spherical-harmonic transforms, providing an artifact-resistant alternative to pixel-space ``ud_grade`` for diffuse signals. Supports pixel-window and beam transfer corrections via the effective beam transfer ratio (Planck Collaboration 2015 X), with ``pixwin=True`` by default. ``fwhm_out=None`` applies the effective resolution beam (Planck-scaled FWHM at the output NSIDE) to suppress Gibbs ringing; ``fwhm_out=0`` (the default) performs plain bandlimit truncation. Custom beam arrays are supported via ``beam_window_in`` / ``beam_window_out``. ``input_type='alm'`` accepts :math:`a_{\ell m}` coefficients directly, skipping the forward SHT. Defaults to ``lmax = 3 * nside_out - 1`` and pixel-weighted analysis, with iteration chosen automatically based on the HEALPix ``lmax <= 1.5 * nside_in`` guidance. Added focused tests, API cross-links from ``ud_grade``, and tutorial notebooks https://github.com/healpy/healpy/pull/1081

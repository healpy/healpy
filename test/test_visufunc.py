...
class TestNoCrash(unittest.TestCase):
    ...
    def test_colormap_object_behavior(self):
        """Test explicit bad/under colors are applied consistently."""
        ...
        # Test 2: Colormap object should also apply explicit badcolor/bgcolor
        cmap_obj = plt.get_cmap('viridis').copy()
        cmap_obj.set_bad('white')
        cmap_obj.set_under('yellow')
        
        cm2 = create_colormap(cmap_obj, badcolor='red', bgcolor='blue')
        bad_is_red = np.allclose(cm2._rgba_bad[:3], [1.0, 0.0, 0.0])
        under_is_blue = np.allclose(cm2._rgba_under[:3], [0.0, 0.0, 1.0])
        assert bad_is_red, "Colormap object should apply badcolor"
        assert under_is_blue, "Colormap object should apply bgcolor"
    ...
    def test_mollview_with_colormap_object(self):
        """Test that mollview applies explicit bad/under colors with Colormap objects."""
        # Create a colormap with custom bad/under colors
        cmap = plt.get_cmap('viridis').copy()
        cmap.set_bad('white')
        cmap.set_under('yellow')
        
        # Call mollview with explicit bad/under colors
        mollview(self.m, cmap=cmap, badcolor='gray', bgcolor='white')
        ...
        if hasattr(ax, 'images') and len(ax.images) > 0:
            ...
            assert bad_is_gray, "mollview should apply badcolor for Colormap objects"
            assert under_is_white, "mollview should apply bgcolor for Colormap objects"
